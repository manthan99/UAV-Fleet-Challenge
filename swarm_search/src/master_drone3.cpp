...
int navigate(ros::NodeHandle nh, geographic_msgs::GeoPoseStamped pose1)
{
  ...
  ros::Publisher global_pos_pub = nh.advertise<geographic_msgs::GeoPoseStamped>("/drone3/mavros/setpoint_position/global", 10);
...
void scan_main(ros::NodeHandle nh)
{
  ...
  ros::Publisher flags_pub = nh.advertise<swarm_search::point_list>("/drone3/flags", 10);
...
int main(int argc, char** argv)
{
  ...
  //ros::Publisher global_pos_pub = nh.advertise<mavros_msgs::GlobalPositionTarget>("/drone3/mavros/setpoint_position/global", 10);
  ros::Publisher global_pos_pub = nh.advertise<geographic_msgs::GeoPoseStamped>("/drone3/mavros/setpoint_position/global", 10);
  ros::Subscriber currentPos = nh.subscribe<sensor_msgs::NavSatFix>("/drone3/mavros/global_position/global", 10, pose_cb);
 
  ros::Subscriber nextGoal = nh.subscribe<geographic_msgs::GeoPoseStamped>("/master/drone3/next_goal/pose", 10, setDestination);
  ros::Subscriber receive_master_cmd = nh.subscribe<mavros_msgs::GlobalPositionTarget>("/master/drone3/received_cmd", 10, receive_cmd);
